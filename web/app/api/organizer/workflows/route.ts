...
const standardTicketTypes = new Set([
  "Regular",
  "VIP",
  "VVIP",
  "Regular Group of 5",
...
function text(fields: Record<string, string>, key: string, min = 1, max = 500) {
  ...
  if (value.length < min || value.length > max)
    throw new Error(
      `Enter a valid ${key.replace(/([A-Z])/g, " $1").toLowerCase()}.`,
    );
  return value;
}
...
function ticketTypeName(fields: Record<string, string>) {
  ...
  if (!standardTicketTypes.has(name))
    throw new Error("Choose a valid Tokea ticket type.");
  return name;
}
...
function ticketTypeRows(fields: Record<string, string>) {
  if (!fields.ticketTypes) {
    return [
      {
        name: ticketTypeName(fields),
        priceKes: fields.priceKes,
  ...
  if (!Array.isArray(parsed))
    throw new Error("Choose valid ticket categories.");
  const rows = parsed
    .map((item) => {
  ...
  if (rows.length === 0)
    throw new Error("Add quantity for at least one ticket category.");
  return rows;
}
...
function jsonRows(
  ...
  if (!Array.isArray(parsed))
    throw new Error(
      `Add valid ${key.replace(/([A-Z])/g, " $1").toLowerCase()}.`,
    );
  const rows = parsed
    .filter(
  ...
  if (rows.length === 0)
    throw new Error(
      `Add at least one ${key.replace(/([A-Z])/g, " $1").toLowerCase()} row.`,
    );
  return rows;
}
...
function shiftWindow(fields: Record<string, string>, startsAt?: string | null) {
  ...
  const end = fields.shiftEnd
    ? new Date(fields.shiftEnd)
    : new Date(start.getTime() + 8 * 60 * 60 * 1000);
  return {
    starts_at: Number.isNaN(start.getTime())
      ? eventStart.toISOString()
      : start.toISOString(),
    ends_at: Number.isNaN(end.getTime())
      ? new Date(eventStart.getTime() + 8 * 60 * 60 * 1000).toISOString()
      : end.toISOString(),
  };
}
...
async function ensureStaffProfile(profileId: string, department?: string) {
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      staff_category: normalizeDepartment(department),
    })
    .select("id")
    .single();
  if (error || !data)
    throw error ?? new Error("Unable to create staff profile.");
...
async function ensureVolunteerProfile(profileId: string) {
  ...
  const { data, error } = await admin
    ...
    .single();
  if (error || !data)
    throw error ?? new Error("Unable to create volunteer profile.");
  return data.id as string;
}
...
async function ensureWorkspace(
  ...
  if (!workspaceId) {
    const { data, error } = await admin
      ...
      .insert({
        ...
        name: `${eventTitle} Workspace`,
      })
      .select("id")
      .single();
    if (error || !data)
      throw error ?? new Error("Unable to create event workspace.");
  ...
  const { error } = await admin
    ...
    .upsert(channels, { onConflict: "workspace_id,kind" });
  if (error) throw error;
  return workspaceId;
}

async function assignStaffToEvent(params: {
  email: string;
  eventId: string;
  organizerId: string;
  eventTitle: string;
  ...
  if (!user)
    return {
      assigned: false,
      reason: "No active account exists for that staff email yet.",
    };
    ...
    .upsert(
      {
        ...
        department: normalizeDepartment(params.department),
        status: "assigned",
      },
      { onConflict: "event_id,staff_profile_id" },
    )
    .select("id")
    .single();
  if (error || !assignment)
    throw error ?? new Error("Unable to assign staff member.");
...
async function assignVolunteerToEvent(params: {
  email: string;
  eventId: string;
  organizerId: string;
  eventTitle: string;
  ...
  if (!user)
    return {
      assigned: false,
      reason: "No active account exists for that volunteer email yet.",
    };
  ...
  const { data: application, error } = await admin
    ...
    .upsert(
      ...
      { onConflict: "event_id,volunteer_profile_id" },
    )
    .select("id")
    .single();
  if (error || !application)
    throw error ?? new Error("Unable to assign volunteer.");
  const window = shiftWindow(params.fields, params.startsAt);
  const { data: existingShift } = await admin
    .from("staff_shifts")
    .select("id")
...
export async function POST(request: NextRequest) {
  ...
  if (eventActions.has(action)) {
    ...
    if (!event)
      return NextResponse.json(
        { error: "That event is not available to your organization." },
        { status: 403 },
      );
  ...
  try {
    ...
    switch (action) {
      case "ticket_type":
        {
          const rows = ticketTypeRows(fields).map((row, index) => ({
            ...
            quantity_total: Number(
              text({ quantity: row.quantity }, "quantity", 1, 7),
            ),
            ...
            rows.push({
              ...
              priority:
                ...
                row.priority === "low"
                  ? row.priority
                  : "medium",
              due_at: optionalTimestamp(row, "dueAt"),
              created_by: auth.user.id,
            });
          }
      ...
      case "staff_invite":
        {
          ...
          for (const row of rows) {
            ...
            if (assignment.assigned) assignedCount += 1;
            else pendingCount += 1;
          }
      ...
      case "volunteer_opportunity":
        {
          ...
          const inserts = rows.map((row) => ({
            event_id: eventId!,
            title: text(row, "title", 2, 120),
            description: row.description?.trim() || null,
            required_count: Number(text(row, "requiredCount", 1, 5)),
            created_by: auth.user.id,
          }));
          const { error: clearError } = await auth.supabase
      ...
      case "vendor_request":
        {
          const rows = jsonRows(fields, "vendorRequests", fields).map(
            (row) => ({
              event_id: eventId!,
              service_category: text(row, "category", 2, 80),
              requirements: text(row, "requirements", 2, 1000),
              budget_cents: row.budgetKes ? amount(row, "budgetKes") : null,
      ...
      case "foodo":
        {
          await auth.supabase
            ...
            .upsert({ event_id: eventId!, foodo_active: true });
          const rows = jsonRows(fields, "foodoVendors", {
            vendorName: fields.foodoBrief || "Foodo vendor",
            cuisineType: fields.foodoBrief || "Street food",
            vendorFeeKes: fields.vendorFeeKes || "0",
            requirements: fields.requirements || "Foodo vendor setup",
            stallNumber: "A1",
            menuSummary: fields.foodoBrief || "Menu to be confirmed",
          });
          let created = 0;
          const admin = createSupabaseAdminClient();
          for (const row of rows) {
            const { data: vendor, error: vendorError } = await admin
              .from("food_vendors")
              .insert({
                vendor_name: text(row, "vendorName", 2, 120),
                cuisine_type: row.cuisineType || null,
                menu_summary: row.menuSummary || null,
              })
              .select("id")
              .single();
            if (vendorError || !vendor)
              throw vendorError ?? new Error("Unable to create Foodo vendor.");
            const stallNumber = text(
              { stallNumber: row.stallNumber || `F${created + 1}` },
              "stallNumber",
              1,
              40,
            );
              ...
              .upsert(
                {
                  ...
                  food_vendor_id: vendor.id,
                  location: row.location || null,
                },
                { onConflict: "event_id,stall_number" },
              )
              .select("id")
              .single();
            if (stallError || !stall)
              throw stallError ?? new Error("Unable to create Foodo stall.");
              ...
              .upsert(
                {
                  ...
                  assigned_stall_id: stall.id,
                  assigned_category: row.cuisineType || null,
                  vendor_fee_cents: row.vendorFeeKes
                    ? amount(row, "vendorFeeKes")
                    : 0,
                },
                { onConflict: "event_id,food_vendor_id" },
              );
        ...
        break;
      case "triplink_route": {
        const { data: providers, error: providerError } = await auth.supabase
        ...
        if (!providerId) {
          const { data: provider, error: createProviderError } =
            await auth.supabase
              .from("transport_providers")
              .insert({
                profile_id: auth.user.id,
                company_name: "Organizer Triplink Routes",
              })
              .select("id")
              .single();
          if (createProviderError || !provider)
            throw (
              createProviderError ??
              new Error("Unable to create a route provider.")
            );
          ...
          const schedules = [
            optionalTimestamp(row, "departureAt")
              ? {
                  ...
                  departs_at: optionalTimestamp(row, "departureAt"),
                }
              : null,
            optionalTimestamp(row, "returnAt")
              ? {
                  label: "Return departure",
                  departs_at: optionalTimestamp(row, "returnAt"),
                }
              : null,
          ].filter(Boolean);
          return {
            ...
            schedules,
            price_cents: amount(row, "priceKes"),
            capacity: Number(text(row, "capacity", 1, 6)),
          };
        });
      ...
      case "sponsorship_package":
        {
          const rows = jsonRows(fields, "sponsorshipPackages", fields).map(
            (row) => ({
              event_id: eventId!,
              name: text(row, "name", 2, 100),
              price_cents: amount(row, "priceKes"),
              benefits: text(row, "benefits", 2, 1000)
                .split("\n")
                .map((item) => item.trim())
                .filter(Boolean),
              inventory_count: Number(text(row, "inventory", 1, 5)),
              created_by: auth.user.id,
            }),
          );
          const { error: deleteError } = await auth.supabase
      ...
      case "budget":
        {
          const rows = jsonRows(fields, "budgets", fields).map((row) => ({
            event_id: eventId!,
            category: text(row, "category", 2, 80),
            budgeted_cents: amount(row, "budgetKes"),
      ...
      case "event_schedule":
        {
          const rows = jsonRows(fields, "scheduleItems", fields).map((row) => {
            const startsAt = dateTimeFromParts(
              row,
              "scheduleDate",
              "startTime",
            );
            if (!startsAt)
              throw new Error("Choose a valid schedule date and start time.");
            const title = text(row, "title", 2, 120);
            if (
              row.title &&
              !scheduleTitles.has(row.title) &&
              row.title.length < 2
            )
              throw new Error("Choose a valid schedule title.");
            return {
              event_id: eventId!,
              title,
              description: row.description?.trim() || null,
              starts_at: startsAt,
              ends_at: dateTimeFromParts(row, "scheduleDate", "endTime"),
              location_label: row.locationLabel?.trim() || null,
        ...
        break;
      case "workspace": {
        const { data: workspace, error: workspaceError } = await auth.supabase
          .from("event_workspaces")
          .upsert(
            {
              event_id: eventId!,
              organizer_id: organizerId!,
              name: `${text(fields, "workspaceName", 2, 120)} Workspace`,
            },
            { onConflict: "event_id" },
          )
          .select("id")
        ...
        if (workspaceError || !workspace)
          throw workspaceError ?? new Error("Unable to open workspace.");
        await ensureWorkspace(eventId!, organizerId!, eventTitle);
        ({ error } = await auth.supabase.from("workspace_channels").upsert(
          [
            {
      ...
      case "organization":
        ({ error } = await auth.supabase
          ...
          .update({
            organization_name: text(fields, "organizationName", 2, 120),
            description: fields.description?.trim() || null,
