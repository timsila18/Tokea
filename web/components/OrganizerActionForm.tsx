...
function ActionFields({ action, fields, setField }: { action: OrganizerAction; fields: Record<string, string>; setField: (name: string, value: string) => void }) {
  ...
  if (action === 'ticket_type') return <div className="workflow-grid"><label>Ticket type<select value={fields.name ?? 'Regular'} onChange={(event) => setField('name', event.target.value)} required>{standardTicketTypes.map((name) => <option key={name} value={name}>{name}</option>)}</select></label>{input('priceKes', 'Price (KES)', 'number')}{input('quantity', 'Quantity', 'number')}{input('salesStart', 'Sales start', 'datetime-local', false)}{input('salesEnd', 'Sales end', 'datetime-local', false)}{textarea('description', 'Description', false)}</div>;
  if (action === 'campaign') return <div className="workflow-grid">{input('name', 'Campaign name')}{input('channel', 'Channel')}{input('startsAt', 'Campaign start', 'datetime-local', false)}{input('endsAt', 'Campaign end', 'datetime-local', false)}{textarea('message', 'Campaign message')}</div>;
  ...
  if (action === 'staff_invite') return <div className="workflow-grid">{input('email', 'Staff email', 'email')}{input('roleTitle', 'Role title')}<label>Department<select value={fields.department ?? 'operations_team'} onChange={(event) => setField('department', event.target.value)}><option value="operations_team">Operations team</option><option value="security">Security</option><option value="ushers">Ushers</option><option value="ticket_scanners">Ticket scanners</option><option value="media_team">Media team</option><option value="customer_support">Customer support</option><option value="vip_coordinators">VIP coordinators</option></select></label>{input('shiftStart', 'Shift start', 'datetime-local', false)}{input('shiftEnd', 'Shift end', 'datetime-local', false)}</div>;
  if (action === 'volunteer_opportunity') return <div className="workflow-grid">{input('title', 'Opportunity title')}{input('requiredCount', 'Volunteers required', 'number')}{input('volunteerEmail', 'Assign volunteer email', 'email', false)}{input('shiftStart', 'Shift start', 'datetime-local', false)}{input('shiftEnd', 'Shift end', 'datetime-local', false)}{textarea('description', 'Opportunity details', false)}</div>;
  ...
  if (action === 'triplink_route') return <div className="workflow-grid">{input('routeName', 'Route name')}{input('departureAt', 'First departure', 'datetime-local', false)}{input('returnAt', 'Return departure', 'datetime-local', false)}{input('priceKes', 'Price per seat (KES)', 'number')}{input('capacity', 'Seats available', 'number')}{input('pickupPoints', 'Pickup points (comma-separated)')}{input('dropoffPoint', 'Dropoff point')}</div>;
