...
const campaignDefaults = [
  { name: 'Launch campaign', channel: 'Instagram', message: 'Launch event announcement and early interest push.', startsAt: '', endsAt: '' },
  { name: 'Final week sales', channel: 'WhatsApp', message: 'Final week ticket reminder and share link.', startsAt: '', endsAt: '' },
];
const taskDefaults = [
...
export function OrganizerActionForm({ action, onClose, initialEventId }: { action: OrganizerAction; onClose: () => void; initialEventId?: string }) {
...
function ActionFields({ action, fields, setField }: { action: OrganizerAction; fields: Record<string, string>; setField: (name: string, value: string) => void }) {
  ...
  if (action === 'campaign') return <BulkRows title="Campaigns" storageKey="campaigns" fields={fields} setField={setField} defaults={campaignDefaults} columns={[{ key: 'name', label: 'Campaign name', options: ['Launch campaign', 'Early bird push', 'Final week sales', 'VIP table push', 'Influencer reel burst'] }, { key: 'channel', label: 'Channel', options: campaignChannels }, { key: 'startsAt', label: 'Campaign start', type: 'datetime-local' }, { key: 'endsAt', label: 'Campaign end', type: 'datetime-local' }, { key: 'message', label: 'Message', type: 'textarea', wide: true }]} />;
...
function BulkInput({ column, value, onChange }: { column: BulkField; value: string; onChange: (value: string) => void }) {
  ...
  return <label className={className}>{column.label}<input className={column.type === 'date' || column.type === 'time' || column.type === 'datetime-local' ? 'picker-input' : undefined} type={column.type ?? 'text'} value={value} onChange={(event) => onChange(event.target.value)} placeholder={column.placeholder} />{(column.type === 'date' || column.type === 'time' || column.type === 'datetime-local') && <span className="picker-help">Click the calendar or clock icon to pick.</span>}</label>;
}
